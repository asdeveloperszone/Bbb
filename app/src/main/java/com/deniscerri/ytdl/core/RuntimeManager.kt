...
object RuntimeManager {
    ...
    fun initYTDLP(appContext: Context, ytdlpDir: File) {
        ...
        if (!ytdlpBinary.exists()) {
            // Try bundled raw resource first (for builds that bundle yt-dlp)
            try {
                val inputStream = appContext.resources.openRawResource(R.raw.ytdlp)
                FileUtils.copyInputStreamToFile(inputStream, ytdlpBinary)
            } catch (e: Exception) {
                // Not bundled — binary is downloaded by SetupActivity on first launch.
                // Do not crash here; RuntimeManager is only called after setup completes.
                android.util.Log.w("RuntimeManager", "yt-dlp not bundled; expected to be downloaded by SetupActivity")
            }
        }
    }
