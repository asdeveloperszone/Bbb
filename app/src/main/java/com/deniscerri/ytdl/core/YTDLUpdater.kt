...
internal object YTDLUpdater {
    ...
    private fun getDownloadUrl(json: JSONObject): String {
        ...
        // Download plain "yt-dlp" — the Python script (no extension)
        // This is NOT a native binary — it's a Python script so Android SDK28+ allows it
        for (i in 0 until assets.length()) {
            val asset = assets.getJSONObject(i)
            if (asset.getString("name") == RuntimeManager.ytdlpBin) {
